...
def main():
    ...
    improb = gen_probfield(imdist,dev=40)
...
def gen_probfield(imarr,dev=25.0,reweight = True):
    prob_arr = np.zeros(imarr.shape,dtype=float)
    for i in range(len(imarr)):
        for j in range(len(imarr[i])):
            prob_arr[i][j] = custom_sigmoid(imarr[i][j],dev)
...
def custom_sigmoid(x,dev=25.0,max_rand = 0.95):
    return min(2 - np.divide(2,1+np.exp(-1*x*np.divide(1,dev*1.5))),max_rand)

# Create a matrix of distances from black pixels
def gen_distfield(imarr,maxdist=1000,every_check=50,upperlim = np.iinfo(np.int32).max):
